...
class MockApiBase(ApiCaller):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.mock_data_dir = os.path.abspath(
                os.path.join(os.path.dirname(__file__), os.path.pardir, "data")
            )
        self.mock_response_pkl = kwargs.get(
            "mock_response_pkl", "mock_success_response.pkl"
        )

    def get_mock_response(self):
        try:
            with open(
                os.path.join(self.mock_data_dir, self.mock_response_pkl), "rb"
            ) as f:
                return pickle.loads(f.read())
        except FileNotFoundError as e:
            raise FileNotFoundError(
                "Pickled mock response not found. Try running script to populate mock data."
            )
...
class MockApiValidation(MockApiBase):
    ...
    resource_optional_params_map = {MOCK_RESOURCE: ("mock_param_1", "mock_param_2")}
...
class TestApiCallerParameterValidation(TestCase):
    ...
    def test_invalid_optional_parameters(self):
        ...
        for invalid_params in optional_params:
            self.assertRaises(
                ValueError, api._validate_optional_params, "mock", optional_params
            )

    def test_no_optional_params_map(self):
        api = MockApiBase()
        self.assertEqual(
            api._validate_optional_params("mock", {"optional_param_1": None}), None
        )
...
class TestApiCallerFetch(TestCase):
    def setUp(self):
        ...
        expected_success_response_pkl = os.path.abspath(
            os.path.join(
